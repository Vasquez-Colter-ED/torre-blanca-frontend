            ...
            <div className="db-section">
              ...
              <div className="db-cuotas-list">
                {cuotasUrgentes.map(c => {
                  const ultimoPago = [...(c.pagos || [])].sort((a, b) => new Date(b.fechaPago) - new Date(a.fechaPago))[0]
                  const fueRechazado = ultimoPago?.estado === 'RECHAZADO'
                  return (
                    <div key={c.cuotaId} className={`db-cuota-card db-cuota-urgent ${c.estadoCuota==='VENCIDO'?'db-cuota-vencida':''} ${fueRechazado ? 'db-cuota-card-conaviso' : ''}`}>
                      {fueRechazado && (
                        <div className="db-rechazo-aviso">
                          <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><path d="M10.29 3.86L1.82 18a2 2 0 0 0 1.71 3h16.94a2 2 0 0 0 1.71-3L13.71 3.86a2 2 0 0 0-3.42 0z"/><line x1="12" y1="9" x2="12" y2="13"/><line x1="12" y1="17" x2="12.01" y2="17"/></svg>
                          <div>
                            <p className="db-rechazo-t">Pago anterior rechazado</p>
                            <p className="db-rechazo-motivo">{ultimoPago.observaciones ? `Motivo: ${ultimoPago.observaciones}` : 'Vuelve a intentarlo con un comprobante válido.'}</p>
                          </div>
                        </div>
                      )}
                      <div className={fueRechazado ? 'db-cuota-fila-interna' : 'db-cuota-fila-interna db-cuota-fila-sinborde'}>
